# Mellanox Technologies - [www.mellanox.com](www.mellanox.com)

__MSTFLINT Package__ - Firmware Burning and Diagnostics Tools
...
>* As of [mstflint-4.8.0](https://github.com/Mellanox/mstflint/releases/tag/v4.6.0-1) `./configure --enable-cs` will be deprecated; instead use `./configure --enable-openssl` which will enable all open SSL features including CS.
>* As of [mstflint-4.6.0](https://github.com/Mellanox/mstflint/releases/tag/v4.6.0-1) `hca_self_test.ofed` will not be a part of msflint package,
...
* __mstflint source code__
* __mflash lib__ This lib provides low level Flash access through Mellanox HCAs.
* __mtcr lib__ (implemented in mtcr.h file) This lib enables access to HCA hardware registers.
* __mstregdump utility__ This utility dumps hardware registers from Mellanox hardware for later analysis by Mellanox.
* __mstvpd__ This utility dumps the on-card VPD.
* __mstmcra__ This debug utility reads/writes a to/from the device configuration register space.
* __mstconfig__ This tool sets or queries non-volatile configurable options for Mellanox HCAs.
* __mstfwmanager__ Mellanox firmware update and query utility which scans the system for available Mellanox devices (only mst PCI devices) and performs the necessary firmware updates.
* __mstreg__ The mlxreg utility allows users to obtain information regarding supported access registers, such as their fields and attributes.
* __mstfwtrace__ The mstfwtrace utility extracts and prints trace messages generated by the firmware running on 5th generation devices iRISCs. This tool supports secure FW flow only.
* __mstlink__ The mstlink tool is used to check and debug link status and issues related to them.

## Installation

Build the mstflint utility. This package is built using a standard autotools method.
> Note that if you are trying to build from the GitHub repository, need to run ./autogen.sh first.
...
The devices can be accessed by their PCI ID as displayed by lspci `(bus:dev.fn)`.
...
When the IB driver (mlx4 or mthca) is loaded, the devices can be accessed by their IB device name.
...
To force configuration access, use device names in the following format: `/proc/bus/pci/<bus>/<dev.fn>`
...
### Accessing a multi-function device

In some configuration, the CA device identifies as a multi-function device on PCI:
...
[Mellanox website firmware section](https://network.nvidia.com/support/firmware/firmware-downloads/) to
